...
for i in range(300):
    N = np.random.randn(npop, 3)  # 產生噪點
    R = np.zeros(npop)
    for j in range(npop):
        w_try = w + sigma*N[j]
        R[j] = f(w_try)             # 得到環境獎勵
    A = (R - np.mean(R)) / np.std(R)  # 歸一化獎勵
    w = w + alpha/(npop*sigma) * np.dot(N.T, A)   # 更新參數

print(w)
# endregion


# region Evolution Strategy with Neural Nets
